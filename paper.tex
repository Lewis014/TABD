...
La literatura científica crece a un ritmo que desborda los métodos de búsqueda
tradicionales. A mayo de 2026, ArXiv supera los 2,6 millones de artículos
\cite{ref1} y PubMed indexa más de 38 millones de registros biomédicos \cite{ref2}.
Sobre colecciones de esa magnitud, la búsqueda por coincidencia de palabras clave
resulta insuficiente: dos abstracts pueden describir la misma idea sin compartir un
solo término. Para salvar esa distancia, los sistemas actuales representan cada
documento como un vector denso de alta dimensionalidad ---un \textit{embedding}---
capaz de capturar similitud semántica más allá del vocabulario literal \cite{ref3}.
La dificultad reaparece al consultar: recuperar el vecino más cercano de forma
exacta exige comparar la consulta contra todos los vectores del corpus, un costo
$O(N \cdot d)$ que crece linealmente con el tamaño de la colección y vuelve la
latencia impracticable sobre millones de documentos. Por ello, la recuperación a
gran escala se apoya en métodos de búsqueda aproximada del vecino más cercano (ANN),
que ceden una fracción mínima de exactitud a cambio de respuestas mucho más veloces
\cite{ref4}.

% Párrafo 2 — Antecedentes: HNSW y los dos factores que rigen su rendimiento
Entre los algoritmos ANN, Hierarchical Navigable Small World (HNSW) \cite{ref5} se
ha consolidado como el estándar de facto para la búsqueda semántica a gran escala.
Inspirado en la teoría de redes de mundo pequeño \cite{ref6}, organiza los vectores
en una jerarquía multi-capa que alcanza una complejidad de consulta $O(\log N)$ con
recalls superiores al 97\% en benchmarks canónicos \cite{ref5}; esta eficiencia
explica su adopción como índice primario en plataformas como FAISS \cite{ref7} y
Milvus \cite{ref8}. Su rendimiento, no obstante, depende de dos factores críticos.
El primero es la parametrización interna: Zhao et al.~\cite{ref9} demuestran que el
ajuste de $M$ y \textit{ef\_construction} gobierna el equilibrio entre la velocidad
de construcción del índice y la exactitud de la recuperación. El segundo es la
métrica de similitud; en el contexto de los sistemas de generación aumentada por
recuperación (RAG) \cite{ref10}, Elkiran y Rasheed \cite{ref11} muestran que, sobre
el corpus SQuAD \cite{ref12}, el coseno alcanza R@10 entre 0.909 y 0.925 mientras
que la distancia euclídea sin normalizar colapsa a 0.373. Ambos resultados, sin
embargo, provienen de colecciones de propósito general ajenas al lenguaje científico
especializado.

% Párrafo 3 — Vacío de conocimiento
A pesar de su amplio estudio, HNSW no se ha evaluado sobre corpus de abstracts
científicos, caracterizados por un vocabulario especializado y una distribución
temática heterogénea. No existe un benchmark que combine modelos de embedding
científico ---como SciBERT o E5-large--- con un análisis controlado del impacto de
$M$ y \textit{ef\_construction} sobre las métricas de recuperación (Recall@K, MRR)
en ese dominio. Esta ausencia limita la transferibilidad de las recomendaciones de
parametrización existentes a contextos reales de recuperación en literatura
científica.

% Párrafo 4 — Objetivo
El presente trabajo evalúa experimentalmente HNSW indexado sobre FAISS para la
búsqueda semántica de abstracts del conjunto \textit{arxiv-abstracts-2021},
comparando los modelos SciBERT y E5-large bajo distintas configuraciones de $M$,
\textit{ef\_construction} y métrica de similitud, con el propósito de identificar la
parametrización óptima que maximice Recall@K y MRR minimizando la latencia de
...
k-NN exacto en espacios de alta dimensionalidad. Mathur y Chhabra \cite{ref4}
clasifican los algoritmos ANN en cuatro familias ---fuerza bruta, árboles,
cuantización y grafos--- y señalan a los métodos basados en grafos como los más
competitivos cuando crece la dimensión. HNSW \cite{ref5} encarna esa familia:
construye una jerarquía multi-capa inspirada en las redes de mundo pequeño
\cite{ref6}, donde las capas altas ofrecen saltos largos y dispersos y la capa base
afina la búsqueda local, con un costo de consulta $O(\log N)$. Sobre esta idea
base han surgido numerosas optimizaciones. Iwasaki y Miyazaki \cite{ref13} ajustan
los grados de entrada y salida de cada nodo ---recortando aristas salientes
redundantes sin perder exactitud--- para acelerar las consultas en datos de alta
dimensión. X. Zhao et al.~\cite{ref14}, por su parte, abordan el costo de
construcción con LSH-APG, un esquema que sustenta la inserción de puntos sobre una
búsqueda ligera basada en hashing sensible a la localidad (LSH) y permite mantener
el grafo de forma incremental conforme el conjunto crece.

La principal limitación de HNSW no reside en la consulta, sino en la construcción
del índice. Cada inserción dispara múltiples recorridos multi-capa y un elevado
número de cálculos de distancia \cite{ref9}. F. Zhao et al.~\cite{ref9} responden
con H-DMax, que introduce un parámetro $\lambda$ para podar vecinos lejanos de
forma jerárquica ---severa en las capas bajas, prudente en las altas para no
romper la conectividad global---, y logran acelerar la construcción hasta
$1.27\times$ perdiendo menos del 0.27\% de Recall@10 sobre SIFT1M
($\lambda=1.10$). La cuantización por producto \cite{ref15} sigue otra ruta:
comprime los vectores en códigos cortos y, dentro de FAISS \cite{ref7}, ofrece la
mayor aceleración de construcción ($3.6\times$). El costo, sin embargo, es alto:
el recall puede caer hasta valores del 85\%, lo que la descarta cuando la
fidelidad no es negociable.

En el plano de la infraestructura, FAISS \cite{ref7} y Milvus \cite{ref8}
constituyen la referencia para desplegar índices HNSW en producción. Milvus admite
coseno, producto interno y L2 sobre embeddings densos e híbridos, y es la base de
numerosos pipelines RAG \cite{ref10} recientes. Sobre esa plataforma, Elkiran y
Rasheed \cite{ref11} evalúan las tres métricas con embeddings de OpenAI
(3072 dimensiones) y el corpus SQuAD \cite{ref12}: coseno y producto interno
alcanzan R@10 $= 0.909$--$0.925$ y MRR $= 0.781$--$0.793$, frente a la fuerte
caída de L2 sin normalizar (R@10 $= 0.373$, MRR $= 0.116$). El umbral de
similitud, como señala la teoría clásica de recuperación de información
\cite{ref16}, modula el equilibrio entre precisión y recall, y la normalización
resulta decisiva cuando se trabaja con L2.

La literatura revisada evidencia una laguna relevante. HNSW no ha sido evaluado
sobre abstracts científicos, caracterizados por un vocabulario denso y una
distribución temática desigual. Tampoco existe un banco de pruebas que compare
modelos de embedding científico como SciBERT o E5-large mientras se exploran
sistemáticamente $M$ y \textit{ef\_construction} contra Recall@K y MRR. Por su
parte, los estudios de tipo RAG se limitan al dominio QA con datasets
generalistas \cite{ref11,ref12}, ajenos a las particularidades semánticas del
texto académico. Ese vacío es el que motiva el presente trabajo.
